...
exports.getDenunciante = (req, res, next) =>{
 ...
 exports.postDenunciante = (req, res, next)=>{
...
exports.getUmDenunciante =  (req, res, next) =>{
    mysql.getConnection((error, conn)=>{
...
exports.ptchDenunciante = (req, res, next) =>{
    mysql.getConnection((error, conn)=>{
        ...
        conn.query(
            'SELECT * FROM Denunciante WHERE email = ?', [req.body.email],(error,results) =>{
                ...
                if(results.length > 0){
                    res.status(409).send({mensagem: 'Denunciante email já cadastrado '})
                }else{
                    bcrypt.hash(req.body.senha, 10, (errBcrypt, hash) => {
                        if(errBcrypt){
                            return res.status(500).send({error: errBcrypt})
...
exports.deleteDenunciante = (req, res, next) =>{
    mysql.getConnection((error, conn)=>{
...
exports.loginDenunciante = (req, res, next) => {
